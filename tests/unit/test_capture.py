...
def test_read_swm_framespeed_matches_sample_swms():
    """Sanity check against the shipped sample tunes — flashitback,
    bronkosaurus, rain8580 are framespeed=1; euphoria is framespeed=2."""
    import os

    base = "/scratch/anarkiwi/pysidwizard/tests/data"
    if not os.path.isdir(base):
        pytest.skip("pysidwizard sample data not available")
    expected = {
        "flashitback.swm": 1,
        "bronkosaurus.swm": 1,
        "euphoria.swm": 2,
    ...
    for name, fs in expected.items():
        assert _read_swm_framespeed(os.path.join(base, name)) == fs, name
